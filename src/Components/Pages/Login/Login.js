...
const Login = () => {
    ...
    if (googleError || error) {
        catchError =
            <small className='text-red-600' >Error:
                {error?.message || googleLoading?.message}
            </small>
    }
                ...
                <div className="w-96  shadow-lg">
                    <div className="card-body">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            {catchError}
                            <input type='submit' value='Login' className="btn bg-accent w-full" />
                        </form>
                        <p>New to agricultural manufacture ? <Link className='text-green-500' to='/signUp'>  Create new account</Link></p>
                        <div className="divider">OR</div>
                        <div className="card-actions justify-center">
                            <button
                                onClick={() => signInWithGoogle()}
                                className="btn bg-accent btn-wide">
                                continue wth google
                            </button>
                        </div>
                    </div>
